...
if(g.llms_txt_status!=="PRESENT_ROOT")a.push({p:1,a:"Add an llms.txt file to your domain root",i:"AI engines will know how to cite your content instead of summarizing without attribution.",s:"llms_txt"});
if((s.application_security.missing_secure_headers||[]).includes("Content-Security-Policy"))a.push({p:2,a:"Add a Content-Security-Policy header",i:"Prevents cross-site scripting and data injection attacks.",s:"csp"});
if(!ae.schema_validation.has_faq_json_ld)a.push({p:3,a:"Add FAQ schema markup",i:"Makes your content eligible for rich results and AI citations.",s:"faq_schema"});
if(se.html_structure.missing_meta_descriptions)a.push({p:4,a:"Add a meta description",i:"Controls how your site appears in search results.",s:"meta_desc"});
if(!se.html_structure.canonical_match)a.push({p:5,a:"Fix your canonical URL",i:"Prevents duplicate content issues.",s:"canonical"});
return a.slice(0,3)}
...
function Badge({good}){return<span style={{fontFamily:mono,fontSize:10,fontWeight:700,letterSpacing:1,padding:"3px 8px",color:good?C.green:C.red,background:good?C.greenGlow:C.redGlow,border:`1px solid ${good?C.green:C.red}22`,borderRadius:2}}>{good?"PASS":"FAIL"}</span>}
function Row({label,value,good,snippet}){const[show,setShow]=useState(false);const hasFix=snippet&&FIX_SNIPPETS[snippet]&&good===false;return<div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"11px 0",borderBottom:`1px solid ${C.blackBorder}`}}><span style={{color:C.gray,fontSize:14}}>{label}</span><div style={{display:"flex",alignItems:"center",gap:8}}>{typeof good==="boolean"?<Badge good={good}/>:<span style={{color:C.white,fontSize:13,fontFamily:mono,fontWeight:600}}>{value}</span>}{hasFix&&<button onClick={()=>setShow(!show)} style={{background:"transparent",border:`1px solid ${show?C.red:C.blackBorder}`,color:show?C.red:C.grayDark,fontSize:9,fontFamily:mono,fontWeight:700,padding:"2px 8px",cursor:"pointer",letterSpacing:1,borderRadius:2}}>{show?"HIDE":"FIX"}</button>}</div></div>{show&&hasFix&&<FixSnippet snippet={FIX_SNIPPETS[snippet]}/>}</div>}
...
function ActionItem({action,index}){const[show,setShow]=useState(false);return<div><div style={{display:"flex",gap:14,alignItems:"flex-start"}}><div style={{fontSize:20,fontWeight:700,fontFamily:mono,color:C.red,lineHeight:1,minWidth:24}}>{index+1}</div><div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}><div style={{fontSize:14,fontWeight:700,color:C.white}}>{action.a}</div>{action.s&&FIX_SNIPPETS[action.s]&&<button onClick={()=>setShow(!show)} style={{background:show?C.redGlow:"transparent",border:`1px solid ${show?C.red:C.blackBorder}`,color:show?C.red:C.grayDark,fontSize:9,fontFamily:mono,fontWeight:700,padding:"2px 8px",cursor:"pointer",letterSpacing:1,borderRadius:2}}>{show?"HIDE CODE":"SHOW FIX"}</button>}</div><div style={{fontSize:13,color:C.gray,lineHeight:1.5,marginTop:3}}>{action.i}</div>{show&&action.s&&FIX_SNIPPETS[action.s]&&<FixSnippet snippet={FIX_SNIPPETS[action.s]}/>}</div></div></div>}
function Insights({data}){const sec=data.security_roots;const geo=data.visibility_canopy.geo_branch;const ins=[];if(geo.llms_txt_status==="MISSING"&&sec.ai_crawl_risk.robots_policy==="PERMISSIVE")ins.push({l:"CRITICAL",t:"AI Crawl Gap",b:"Robots.txt is permissive with no llms.txt. AI scrapers can ingest your content without citation guidance."});if(geo.llms_txt_status==="MISSING"&&sec.ai_crawl_risk.robots_policy!=="RESTRICTIVE")ins.push({l:"WARNING",t:"No LLMs.txt",b:"No llms.txt found. Your content may be summarized by AI engines but never cited."});if((sec.application_security.exposed_endpoints||[]).length>0)ins.push({l:"CRITICAL",t:"Exposed Endpoints",b:`${sec.application_security.exposed_endpoints.length} path(s) accessible: ${sec.application_security.exposed_endpoints.slice(0,4).join(", ")}`});if(sec.business_logic_gaps?.data_provenance_leak)ins.push({l:"WARNING",t:"Provenance Risk",b:"Your content allows AI training sets to ingest without linking back."});if(!ins.length)return null;return<div style={{background:C.blackCard,border:`1px solid ${C.redBorder}`,padding:24,marginBottom:20,borderRadius:6}}><h3 style={{margin:"0 0 20px",fontSize:14,fontWeight:700,color:C.red,fontFamily:heading,textTransform:"uppercase",letterSpacing:1}}>Cross-Reference Intelligence</h3><div style={{display:"flex",flexDirection:"column",gap:16}}>{ins.map((i,x)=><div key={x} style={{borderLeft:`3px solid ${i.l==="CRITICAL"?C.red:C.amber}`,paddingLeft:16}}><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:4}}><span style={{fontSize:9,fontWeight:800,fontFamily:mono,letterSpacing:1.5,color:i.l==="CRITICAL"?C.red:C.amber}}>{i.l}</span><span style={{fontSize:14,fontWeight:700,color:C.white}}>{i.t}</span></div><p style={{margin:0,fontSize:13,color:C.gray,lineHeight:1.65}}>{i.b}</p></div>)}</div></div>}
function EmailGate({onSubmit,onClose}){const[email,setEmail]=useState("");const[name,setName]=useState("");const[sending,setSending]=useState(false);const ref=useRef(null);useEffect(()=>{ref.current?.focus()},[]);const submit=async()=>{if(!email.includes("@"))return;setSending(true);await onSubmit(email,name);setSending(false)};return<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:20}} onClick={onClose}><motion.div initial={{opacity:0,scale:0.95}} animate={{opacity:1,scale:1}} transition={{duration:0.2}} style={{background:C.blackCard,border:`1px solid ${C.blackBorder}`,padding:40,maxWidth:420,width:"100%",borderRadius:8}} onClick={e=>e.stopPropagation()}><h2 style={{fontSize:22,fontWeight:700,color:C.white,margin:"0 0 6px",fontFamily:heading}}>Get your <span style={{color:C.red}}>report</span></h2><p style={{color:C.gray,fontSize:14,margin:"0 0 28px",lineHeight:1.7}}>Enter your email to download the full PDF.</p><div style={{display:"flex",flexDirection:"column",gap:12}}><input type="text" value={name} onChange={e=>setName(e.target.value)} placeholder="Your name (optional)" style={{background:C.black,border:`1px solid ${C.blackBorder}`,color:C.white,padding:"14px 16px",fontSize:14,outline:"none",borderRadius:4}}/><input ref={ref} type="email" value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&submit()} placeholder="you@company.com" style={{background:C.black,border:`1px solid ${C.blackBorder}`,color:C.white,padding:"14px 16px",fontSize:14,fontFamily:mono,outline:"none",borderRadius:4}}/><button onClick={submit} disabled={sending||!email.includes("@")} style={{background:sending?C.grayDark:C.red,border:"none",color:C.white,fontWeight:700,fontSize:14,padding:"16px 32px",cursor:sending?"default":"pointer",letterSpacing:1,opacity:!email.includes("@")?0.5:1,borderRadius:4}}>{sending?"PROCESSING...":"DOWNLOAD REPORT"}</button></div><p style={{color:C.grayDark,fontSize:10,marginTop:16,fontFamily:mono}}>No spam. Email used only for this report.</p></motion.div></div>}
...
export default function CanopyGuard(){
  ...
  const handleEmail=useCallback(async(email,name)=>{await storeLead(email,report,name);sendReportEmail(email,report,name);setCapturedEmail(email);setLeadCaptured(true);setShowGate(false);downloadPDF(report,email)},[report]);
  ...
  {/* Nav */}
  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 24px",borderBottom:`1px solid ${C.blackBorder}`}}><span style={{fontSize:14,fontWeight:700,fontFamily:heading,cursor:"pointer"}} onClick={reset}>CANOPY<span style={{color:C.red}}>GUARD</span></span><div style={{display:"flex",gap:8}}>{!leadCaptured?<button onClick={()=>setShowGate(true)} style={{background:C.red,border:"none",color:C.white,padding:"6px 14px",fontSize:11,fontWeight:700,cursor:"pointer",letterSpacing:1,borderRadius:3}}>GET REPORT</button>:<button onClick={()=>downloadPDF(report,capturedEmail)} style={{background:"transparent",border:`1px solid ${C.green}`,color:C.green,padding:"6px 14px",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:mono,borderRadius:3}}>↓ PDF</button>}<button onClick={()=>setShowMethodology(true)} style={{background:"transparent",border:`1px solid ${C.blackBorder}`,color:C.grayDark,padding:"6px 14px",fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:mono,letterSpacing:1,borderRadius:3}}>DOCS</button><button onClick={reset} style={{background:"transparent",border:`1px solid ${C.blackBorder}`,color:C.gray,padding:"6px 14px",fontSize:12,fontWeight:600,cursor:"pointer",borderRadius:3}}>New Scan</button></div></div>
  <div style={{maxWidth:900,margin:"0 auto",padding:"32px 20px"}}>
  <div style={{marginBottom:32}}><p style={{color:C.grayDark,fontSize:10,fontFamily:mono,letterSpacing:2,margin:"0 0 8px"}}>AUDIT REPORT</p><h1 style={{fontSize:"clamp(24px,4vw,36px)",fontWeight:700,margin:"0 0 4px",fontFamily:heading,letterSpacing:-1}}>{d.target_domain}</h1><p style={{color:C.grayDark,fontSize:11,fontFamily:mono}}>{new Date(d.timestamp).toLocaleString()} · {d.audit_id.slice(0,8)} · {d.scan_duration_ms}ms</p></div>
  {/* Top Actions */}
  {actions.length>0&&<div style={{marginBottom:24,padding:24,background:C.blackCard,border:`1px solid ${C.redBorder}`,borderRadius:6}}><h3 style={{fontSize:14,fontWeight:700,color:C.red,fontFamily:heading,textTransform:"uppercase",letterSpacing:1,margin:"0 0 16px"}}>Top Actions</h3><div style={{display:"flex",flexDirection:"column",gap:14}}>{actions.map((a,i)=><ActionItem key={i} action={a} index={i}/>)}</div></div>}
  {/* Scores */}
  <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{duration:0.5}} style={{display:"flex",alignItems:"flex-end",justifyContent:"center",flexWrap:"wrap",gap:40,padding:"40px 24px",marginBottom:24,border:`1px solid ${C.blackBorder}`,background:C.blackCard,borderRadius:6}}>
    <div style={{textAlign:"center"}}><div style={{fontSize:72,fontWeight:700,fontFamily:mono,color:scoreColor(overallPct),lineHeight:1}}>{overallPct}</div><div style={{fontSize:9,fontWeight:700,color:C.gray,letterSpacing:2,marginTop:8}}>OVERALL</div></div>
    <div style={{width:1,height:60,background:C.blackBorder}}/>
    <ScoreBlock score={s.seo_score} label="SEO" delay={200}/><ScoreBlock score={s.aeo_score} label="AEO" delay={400}/><ScoreBlock score={s.geo_score} label="GEO" delay={600}/><ScoreBlock score={s.security_posture_score} label="SECURITY" delay={800}/>
  </motion.div>
  <Insights data={d}/>
  {/* Detail Grid */}
  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:16,marginBottom:20}}>
    <Section title="SEO" tag="CANOPY" desc="How search engines crawl, index, and rank your site."><Row label="Crawlable" good={d.visibility_canopy.seo_branch.crawlability}/><Row label="H1 Tags" value={d.visibility_canopy.seo_branch.html_structure.h1_count}/><Row label="Meta Descriptions" good={!d.visibility_canopy.seo_branch.html_structure.missing_meta_descriptions} snippet="meta_desc"/><Row label="Canonical Match" good={d.visibility_canopy.seo_branch.html_structure.canonical_match} snippet="canonical"/><Row label="Link Depth" value={`${d.visibility_canopy.seo_branch.internal_linking_depth} clicks`}/>{d.visibility_canopy.seo_branch.html_structure.title&&<Row label="Page Title" value={d.visibility_canopy.seo_branch.html_structure.title.slice(0,40)}/>}</Section>
    <Section title="AEO" tag="CANOPY" desc="How well AI answer engines can extract and cite your content."><Row label="FAQ Schema" good={d.visibility_canopy.aeo_branch.schema_validation.has_faq_json_ld} snippet="faq_schema"/><Row label="Org Schema" good={d.visibility_canopy.aeo_branch.schema_validation.has_organization_json_ld} snippet="org_schema"/><Row label="Any JSON-LD" good={d.visibility_canopy.aeo_branch.schema_validation.has_any_json_ld}/><Row label="Validation Errors" value={d.visibility_canopy.aeo_branch.schema_validation.validation_errors.length||"None"}/><Row label="Q&A Density" value={`${(d.visibility_canopy.aeo_branch.qa_density_score*100).toFixed(0)}%`}/></Section>
    <Section title="GEO" tag="CANOPY" desc="How generative AI models chunk, cite, and surface your pages."><Row label="Chunking Efficiency" value={`${(d.visibility_canopy.geo_branch.chunking_efficiency*100).toFixed(0)}%`}/><Row label="Citation Precision" value={`${(d.visibility_canopy.geo_branch.citation_metrics.precision_rate*100).toFixed(0)}%`}/><Row label="Market Share Gap" value={`${(d.visibility_canopy.geo_branch.citation_metrics.market_share_gap*100).toFixed(0)}%`}/><Row label="llms.txt" good={d.visibility_canopy.geo_branch.llms_txt_status==="PRESENT_ROOT"} snippet="llms_txt"/></Section>
    <Section title="Security" tag="ROOTS" desc="Headers, certificates, and configurations protecting your infrastructure."><Row label="TLS Valid" good={sec.tls?.valid}/><Row label="HSTS" good={sec.tls?.hsts} snippet="hsts"/><Row label="HTTPS Redirect" good={sec.tls?.redirectsToHttps}/><Row label="Robots Policy" value={sec.ai_crawl_risk.robots_policy}/><Row label="Agent Spoofing" good={!sec.ai_crawl_risk.spoofed_agent_vulnerability}/><Row label="Rate Limiting" good={sec.ai_crawl_risk.rate_limiting_active}/><Row label="Exposed Endpoints" value={(sec.application_security.exposed_endpoints||[]).length||"None"}/><Row label="Missing Headers" value={(sec.application_security.missing_secure_headers||[]).length||"None"}/><Row label="Known CVEs" value={(sec.application_security.vulnerabilities||[]).length||"None"}/></Section>
  </div>
  {/* Compliance */}
  <div style={{padding:24,background:C.blackCard,border:`1px solid ${C.blackBorder}`,marginBottom:20,borderRadius:6}}><h4 style={{fontSize:12,fontWeight:700,color:C.white,fontFamily:heading,textTransform:"uppercase",letterSpacing:1,margin:"0 0 16px"}}>Compliance Quick Check</h4><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:8}}>{compliance.map(c=><div key={c.label} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",background:c.pass?C.greenGlow:C.redGlow,border:`1px solid ${c.pass?C.green:C.red}22`,borderRadius:3}}><span style={{fontFamily:mono,fontSize:11,fontWeight:700,color:c.pass?C.green:C.red}}>{c.pass?"✓":"✗"}</span><span style={{fontSize:12,color:C.muted}}>{c.label}</span></div>)}</div></div>
  {/* Missing Headers */}
  {(sec.application_security.missing_secure_headers||[]).length>0&&<div style={{padding:20,background:C.blackCard,border:`1px solid ${C.blackBorder}`,marginBottom:20,borderRadius:6}}><h4 style={{margin:"0 0 12px",fontSize:12,fontWeight:700,color:C.red,letterSpacing:1,textTransform:"uppercase"}}>Missing Headers</h4><div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:12}}>{sec.application_security.missing_secure_headers.map(h=><code key={h} style={{padding:"4px 10px",fontSize:11,fontFamily:mono,fontWeight:600,background:C.redGlow,color:C.red,border:`1px solid ${C.red}33`,borderRadius:2}}>{h}</code>)}</div>{sec.application_security.missing_secure_headers.map(h=>{const k=HEADER_SNIPPET_MAP[h];return k&&FIX_SNIPPETS[k]?<FixSnippet key={h} snippet={FIX_SNIPPETS[k]}/>:null})}</div>}
  {/* CTA */}
  <div style={{textAlign:"center",padding:48,background:C.blackCard,border:`1px solid ${C.blackBorder}`,marginBottom:32,borderRadius:6}}>
    {!leadCaptured?<><h2 style={{fontSize:28,fontWeight:700,margin:"0 0 8px",fontFamily:heading}}>Get your full <span style={{color:C.red}}>report</span></h2><p style={{color:C.gray,fontSize:14,margin:"0 0 28px",maxWidth:440,marginLeft:"auto",marginRight:"auto",lineHeight:1.7}}>Download a PDF with your complete audit, top actions, and compliance check.</p><button onClick={()=>setShowGate(true)} style={{background:C.red,color:C.white,border:"none",fontWeight:700,fontSize:14,padding:"18px 44px",letterSpacing:1,cursor:"pointer",fontFamily:heading,borderRadius:4}}>GET YOUR REPORT</button></>:<><div style={{fontSize:36,marginBottom:12,color:C.green}}>✓</div><h2 style={{fontSize:28,fontWeight:700,margin:"0 0 8px",fontFamily:heading}}>Report delivered to <span style={{color:C.red}}>{capturedEmail}</span></h2><p style={{color:C.gray,fontSize:14,margin:"0 0 28px",maxWidth:460,marginLeft:"auto",marginRight:"auto",lineHeight:1.7}}>Every finding is something we resolve for clients every week. Book a free walkthrough.</p><a href="https://calendly.com/hello-merakislove/new-meeting" target="_blank" rel="noopener noreferrer" style={{display:"inline-block",background:C.red,color:C.white,fontWeight:700,fontSize:14,padding:"18px 44px",textDecoration:"none",letterSpacing:1,fontFamily:heading,borderRadius:4}}>BOOK A FREE WALKTHROUGH</a><div style={{marginTop:16}}><button onClick={()=>downloadPDF(report,capturedEmail)} style={{background:"transparent",border:"none",color:C.gray,fontSize:12,cursor:"pointer",fontFamily:mono,textDecoration:"underline"}}>Download PDF again</button></div></>}
  ...
  <div style={{textAlign:"center",padding:"20px 0",borderTop:`1px solid ${C.blackBorder}`}}><p style={{fontSize:11,color:C.grayDark,margin:"0 0 4px"}}>Canopy Guard · Built by Soulful Tech™ · merakislove.com</p><a href="/privacy.html" style={{fontSize:11,color:C.grayDark,textDecoration:"underline"}}>Privacy Policy</a></div>
  </div></div>}
