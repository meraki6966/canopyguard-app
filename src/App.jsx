...
function getTopActions(report) {
  ...
  if (geo.llms_txt_status !== "PRESENT_ROOT")
    actions.push({ priority: 1, action: "Add an llms.txt file to your domain root", impact: "AI engines will know how to cite your content instead of summarizing without attribution." });
  if ((sec.application_security.missing_secure_headers || []).includes("Content-Security-Policy"))
    actions.push({ priority: 2, action: "Add a Content-Security-Policy header", impact: "Prevents cross-site scripting and data injection attacks against your visitors." });
  if (!aeo.schema_validation.has_faq_json_ld)
    actions.push({ priority: 3, action: "Add FAQ schema markup", impact: "Makes your content eligible for rich search results and AI answer engine citations." });
  if (seo.html_structure.missing_meta_descriptions)
    actions.push({ priority: 4, action: "Add a meta description to your homepage", impact: "Controls how your site appears in search results. Without it, engines guess." });
  if (!seo.html_structure.canonical_match)
    actions.push({ priority: 5, action: "Fix your canonical URL", impact: "Prevents duplicate content issues that dilute your search rankings." });
  if ((sec.application_security.missing_secure_headers || []).includes("Strict-Transport-Security"))
    actions.push({ priority: 6, action: "Enable HSTS header", impact: "Forces browsers to always use HTTPS, preventing downgrade attacks." });
  if (sec.ai_crawl_risk.robots_policy === "PERMISSIVE" || sec.ai_crawl_risk.robots_policy === "NONE")
    actions.push({ priority: 7, action: "Configure robots.txt for AI crawlers", impact: "Control which AI models can scrape your site and how they use your content." });

  return actions.slice(0, 3);
}

function getComplianceChecks(report) {
  const html_data = report.visibility_canopy.seo_branch;
  const sec = report.security_roots;
  return [
    { label: "HTTPS Active", pass: sec.tls?.valid || false },
    { label: "Privacy Policy Page", pass: false }, // would need a /privacy check
...
.pass{color:#4CAF50}.fail{color:#E53935}.warn{color:#FF9800}
...
function Row({label,value,good}) {
  return <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:`1px solid ${C.blackBorder}`}}>
    <span style={{color:C.gray,fontSize:13,fontFamily:body}}>{label}</span>
    {typeof good==="boolean"?<Badge good={good}/>:<span style={{color:C.white,fontSize:13,fontFamily:mono,fontWeight:600}}>{value}</span>}</div>;
}
...
export default function CanopyGuard() {
  ...
  const startScan=async()=>{
    const cleaned=domain.replace(/^https?:\/\//,"").replace(/\/+$/,"").trim();if(!cleaned)return;
  ...
  if(phase==="landing") {
    return <div style={{minHeight:"100vh",background:C.black,fontFamily:body,color:C.white}}>
      ...
      <section style={{textAlign:"center",padding:"100px 24px 80px",maxWidth:800,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",maxWidth:560,margin:"0 auto",border:`2px solid ${C.blackBorder}`,background:C.blackLight,transition:"border-color 0.2s"}}
          onFocus={()=>{}} >
          ...
          <button onClick={startScan} style={{background:C.red,border:"none",color:C.white,fontWeight:900,fontSize:14,padding:"20px 36px",cursor:"pointer",letterSpacing:2,fontFamily:display,transition:"background 0.15s"}}
          ...
          <div style={{background:C.blackCard,border:`1px solid ${C.blackBorder}`,padding:32}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {["SEO: crawlability, H1s, meta, canonicals","AEO: schema markup, FAQ JSON-LD, Q&A density","GEO: chunking efficiency, citation precision, llms.txt"].map(t=>
                <div key={t} style={{fontSize:12,color:C.muted,fontFamily:mono,paddingLeft:12,borderLeft:`2px solid ${C.blackBorder}`}}>{t}</div>)}
            </div>
          </div>
          {/* ROOTS */}
          <div style={{background:C.blackCard,border:`1px solid ${C.blackBorder}`,padding:32}}>
          ...
          <div style={{background:C.blackCard,border:`1px solid ${C.redBorder}`,padding:32}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {["AI scraping without citation guidance","Exposed API routes indexed by RAG parsers","Content provenance leaks to training sets"].map(t=>
                <div key={t} style={{fontSize:12,color:C.muted,fontFamily:mono,paddingLeft:12,borderLeft:`2px solid ${C.red}44`}}>{t}</div>)}
            </div>
          </div>
        </div>
      </section>
        ...
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:32,marginBottom:40}}>
          {[{n:"CISSP",d:"Certified Information Systems Security Professional"},{n:"12+",d:"Production apps audited and deployed"},{n:"5",d:"Active SaaS products in production"}].map(b=>
            <div key={b.n} style={{textAlign:"center"}}>
              ...
              <div style={{fontSize:11,color:C.grayDark,maxWidth:180,marginTop:4}}>{b.d}</div>
            </div>)}
        </div>
        <p style={{fontSize:14,color:C.gray,lineHeight:1.7,maxWidth:520,margin:"0 auto"}}>
          Built by Adam McClarin, a CISSP-certified engineer who builds, deploys, and secures production applications every day. Not a theoretical tool. A practical one, built from real audit work across real client sites.
        </p>
      </section>
  ...
  return <div style={{minHeight:"100vh",background:C.black,fontFamily:body,color:C.white}}>
    ...
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 24px",borderBottom:`1px solid ${C.blackBorder}`}}>
      <span style={{fontSize:14,fontWeight:900,fontFamily:display}}>CANOPY<span style={{color:C.red}}>GUARD</span></span>
    ...
    <div style={{maxWidth:900,margin:"0 auto",padding:"32px 20px"}}>
      ...
      {actions.length>0&&<div style={{marginBottom:24,padding:24,background:C.blackCard,border:`1px solid ${C.redBorder}`}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          {actions.map((a,i)=><div key={i} style={{display:"flex",gap:14,alignItems:"flex-start"}}>
            <div style={{fontSize:20,fontWeight:900,fontFamily:mono,color:C.red,lineHeight:1,minWidth:24}}>{i+1}</div>
            <div><div style={{fontSize:14,fontWeight:700,color:C.white,marginBottom:2}}>{a.action}</div>
              <div style={{fontSize:12,color:C.gray,lineHeight:1.5}}>{a.impact}</div></div></div>)}</div></div>}
      ...
      <div style={{display:"flex",alignItems:"flex-end",justifyContent:"center",flexWrap:"wrap",gap:40,padding:"40px 24px",marginBottom:24,border:`1px solid ${C.blackBorder}`,background:C.blackCard}}>
        <div style={{textAlign:"center"}}>
          <div style={{fontSize:72,fontWeight:900,fontFamily:mono,color:scoreColor(overallPct),lineHeight:1}}>{overallPct}</div>
        ...
        <ScoreBlock score={s.seo_score} label="SEO" delay={200}/>
        <ScoreBlock score={s.aeo_score} label="AEO" delay={400}/>
        <ScoreBlock score={s.geo_score} label="GEO" delay={600}/>
        <ScoreBlock score={s.security_posture_score} label="SECURITY" delay={800}/>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:16,marginBottom:20}}>
        <Section title="SEO" tag="CANOPY" desc="How search engines crawl, index, and rank your site.">
          ...
          <Row label="Meta Descriptions" good={!d.visibility_canopy.seo_branch.html_structure.missing_meta_descriptions}/>
          <Row label="Canonical Match" good={d.visibility_canopy.seo_branch.html_structure.canonical_match}/>
        ...
        <Section title="AEO" tag="CANOPY" desc="How well AI answer engines can extract and cite your content.">
          <Row label="FAQ Schema" good={d.visibility_canopy.aeo_branch.schema_validation.has_faq_json_ld}/>
          <Row label="Org Schema" good={d.visibility_canopy.aeo_branch.schema_validation.has_organization_json_ld}/>
        ...
        <Section title="GEO" tag="CANOPY" desc="How generative AI models chunk, cite, and surface your pages.">
          ...
          <Row label="llms.txt" good={d.visibility_canopy.geo_branch.llms_txt_status==="PRESENT_ROOT"}/>
        </Section>
        <Section title="Security" tag="ROOTS" desc="The headers, certificates, and configurations protecting your infrastructure.">
          ...
          <Row label="HSTS" good={sec.tls?.hsts}/>
      ...
      {(sec.application_security.missing_secure_headers||[]).length>0&&<div style={{padding:20,background:C.blackCard,border:`1px solid ${C.blackBorder}`,marginBottom:20}}>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
          {sec.application_security.missing_secure_headers.map(h=><code key={h} style={{padding:"4px 10px",fontSize:11,fontFamily:mono,fontWeight:600,background:C.redGlow,color:C.red,border:`1px solid ${C.red}33`}}>{h}</code>)}</div></div>}
