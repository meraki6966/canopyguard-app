...
function MethodologyPage({ onBack }) {
  const S = (props) => <div style={{background:C.blackCard,border:`1px solid ${C.blackBorder}`,padding:28,marginBottom:16,...(props.style||{})}}>{props.children}</div>;
  const H = ({children}) => <h3 style={{fontSize:18,fontWeight:900,color:C.white,fontFamily:display,margin:"0 0 14px 0"}}>{children}</h3>;
  const P = ({children}) => <p style={{fontSize:15,color:C.muted,lineHeight:1.8,fontFamily:body,margin:"0 0 10px 0"}}>{children}</p>;
  const W = ({label,val}) => <div style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:`1px solid ${C.blackBorder}`}}>
  ...
  return <div style={{minHeight:"100vh",background:C.black,fontFamily:body,color:C.white}}>
    ...
    <div style={{maxWidth:760,margin:"0 auto",padding:"40px 20px"}}>
      ...
      <p style={{fontSize:16,color:C.muted,lineHeight:1.7,margin:"0 0 40px 0"}}>How every score is calculated, what the weights are, and why. Published so you can verify, challenge, and improve the methodology.</p>

      <S><H>SEO Score (0 to 100)</H>
        <P>Measures how well search engines can crawl, index, and rank your site. 14 signals weighted toward fundamentals that gate everything else. Scoring 100 requires fast response time, 1500+ words of content, 20+ internal links, perfect meta tags, and a sitemap.</P>
        <W label="Crawlable (site returns HTML)" val="0.10"/>
        <W label="Exactly 1 H1 tag (multiple penalized)" val="0.10"/>
        <W label="Meta description present + ideal length (120-160 chars)" val="0.10"/>
        <W label="Title tag present + ideal length (30-60 chars)" val="0.10"/>
        <W label="Canonical URL matches domain" val="0.08"/>
        <W label="Mobile viewport meta tag" val="0.05"/>
        <W label="HTML lang attribute" val="0.03"/>
        <W label="Image alt text coverage (ratio with alt)" val="0.08"/>
        <W label="Word count (gradient: 200/500/1500+)" val="0.10"/>
        <W label="Internal link count (gradient: 5/10/20+)" val="0.10"/>
        <W label="Sitemap.xml exists" val="0.06"/>
        <W label="Response time (under 1s/3s/3s+)" val="0.05"/>
        <W label="H2 heading structure (3+ sections)" val="0.05"/></S>

      <S><H>AEO Score (0 to 100)</H>
        <P>Measures how well AI answer engines can extract and cite your content. 10 signals. Scoring 100 requires multiple JSON-LD types, 5+ FAQ items, Organization and LocalBusiness schema, breadcrumbs, and strong Q&A content density.</P>
        <W label="Any JSON-LD structured data present" val="0.10"/>
        <W label="Organization schema" val="0.12"/>
        <W label="FAQ schema present" val="0.10"/>
        <W label="FAQ item count (gradient: 1/3/5+ items)" val="0.12"/>
        <W label="LocalBusiness or service schema" val="0.08"/>
        <W label="Breadcrumb schema" val="0.06"/>
        <W label="Schema type diversity (1/2/4+ types)" val="0.10"/>
        <W label="Zero validation errors in JSON-LD" val="0.10"/>
        <W label="Q&A density (question sentences vs total)" val="0.12"/>
        <W label="JSON-LD block count (1/2/3+ blocks)" val="0.10"/></S>

      <S><H>GEO Score (0 to 100)</H>
        <P>Measures how well generative AI models can chunk, retrieve, and cite your pages. 8 signals based on how retrieval-augmented generation systems process content. Scoring 100 requires well-structured content with headings, lists, tables, a substantial llms.txt file, and 1500+ words of specific, citable claims.</P>
        <W label="Chunking efficiency (headings, paragraphs, lists, tables)" val="0.25"/>
        <W label="Citation precision (specific data vs generic text)" val="0.20"/>
        <W label="llms.txt present at domain root + length bonus (200+ chars)" val="0.23"/>
        <W label="Content depth by word count (200/500/1500+)" val="0.10"/>
        <W label="Lists present (ordered or unordered)" val="0.05"/>
        <W label="Tables present" val="0.04"/>
        <W label="Heading-to-content ratio (ideal: 1 per ~150 words)" val="0.08"/>
        <W label="Baseline (site is reachable)" val="0.05"/>
        <P>Chunking efficiency factors in heading count (target 8+), paragraph count (target 12+), presence of lists and tables, and content length. Well-structured pages with clear headers and short paragraphs produce cleaner retrieval chunks. Citation precision measures the ratio of specific data points (numbers, proper nouns, dates, currencies, percentages) to total word count. Pages with verifiable claims get cited more often than generic copy.</P></S>

      <S><H>Security Score (0 to 100)</H>
        <P>Measures the external security posture visible from outside the site. 15 signals. Individual security headers are weighted separately based on protective scope. Scoring 100 requires all headers, HSTS with 1-year max-age, HTTPS redirect, balanced AI crawl policy, rate limiting, and secure cookie flags.</P>
        <W label="TLS certificate valid" val="0.10"/>
        <W label="HSTS header present" val="0.05"/>
        <W label="HSTS max-age 1 year+ bonus" val="0.03"/>
        <W label="HTTP redirects to HTTPS" val="0.08"/>
        <W label="Content-Security-Policy" val="0.08"/>
        <W label="Strict-Transport-Security" val="0.06"/>
        <W label="X-Frame-Options" val="0.05"/>
        <W label="X-Content-Type-Options" val="0.04"/>
        <W label="Referrer-Policy" val="0.04"/>
        <W label="Permissions-Policy" val="0.04"/>
        <W label="Cookie security flags (Secure, HttpOnly, SameSite)" val="0.06"/>
        <W label="AI crawl policy BALANCED or RESTRICTIVE" val="0.08"/>
        <W label="No spoofed agent vulnerability" val="0.06"/>
        <W label="Rate limiting active" val="0.04"/>
        <W label="Sitemap reference in robots.txt" val="0.02"/>
        <W label="No exposed endpoints" val="0.08"/>
        <W label="No known vulnerability indicators" val="0.04"/>
        <W label="Data provenance (author attribution)" val="0.04"/></S>
      ...
      <S><H>12 Scan Modules</H>
        ...
        {["DNS Resolution (Google Public DNS API)","TLS and Certificate Validation","Security Header Scan (6 headers)","HTML Structure Parse (H1, meta, canonical, title)",
          ...
          <div key={i} style={{display:"flex",gap:10,padding:"6px 0"}}><span style={{color:C.red,fontFamily:mono,fontSize:13,fontWeight:800,minWidth:24}}>{String(i+1).padStart(2,"0")}</span>
            <span style={{color:C.muted,fontSize:14,fontFamily:body}}>{m}</span></div>)}</S>
